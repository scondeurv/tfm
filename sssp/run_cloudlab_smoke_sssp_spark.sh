...
INPUT_KEY="${SPARK_SMOKE_INPUT_KEY:-cloudlab/spark/sssp/large_100000.txt}"
OUTPUT_PREFIX="${SPARK_SMOKE_OUTPUT_PREFIX:-cloudlab/spark/sssp/large-100000/output}"
SOURCE_NODE="${SSSP_SPARK_SMOKE_SOURCE:-0}"
MAX_ITER="${SSSP_SPARK_SMOKE_MAX_ITER:-500}"
PARTITIONS="${SSSP_SPARK_SMOKE_PARTITIONS:-4}"
GRAPH_FILE="${SSSP_SPARK_SMOKE_GRAPH_FILE:-${SCRIPT_DIR}/large_sssp_100000.txt}"
NODES="${SSSP_SPARK_SMOKE_NODES:-100000}"
...
REMOTE_RUNNER="${REMOTE_RUNNER:-/tmp/${SPARK_NAMESPACE}/run-remote-sssp-smoke.sh}"
REMOTE_BASE="${REMOTE_BASE:-/tmp/${SPARK_NAMESPACE}}"
REMOTE_GRAPH_FILE="${REMOTE_BASE}/$(basename "${GRAPH_FILE}")"
LOCAL_OUTPUT_BASE="$(mktemp -d /tmp/${SPARK_NAMESPACE}-sssp-local-XXXXXX)"
LOCAL_RUN_LOG="${LOCAL_OUTPUT_BASE}/remote-run.log"
REMOTE_OUTPUT_DIR="/tmp/${SPARK_NAMESPACE}-sssp-output"
REMOTE_OUTPUT_TAR="/tmp/${SPARK_NAMESPACE}-sssp-output.tgz"
...
scp -i "${CLOUDLAB_SSH_KEY}" \
  "${ROOT_DIR}/baselines/cloudlab/run-remote-sssp-smoke.sh" \
...
VALIDATION_JSON="$("${PYTHON_BIN}" "${ROOT_DIR}/baselines/cloudlab_spark_smoke_validate.py" sssp \
  --local-output-dir "${LOCAL_OUTPUT_DIR}/$(basename "${REMOTE_OUTPUT_DIR}")" \
  --graph-file "${GRAPH_FILE}" \
  --num-nodes "${NODES}" \
  --source-node "${SOURCE_NODE}" || true)"
