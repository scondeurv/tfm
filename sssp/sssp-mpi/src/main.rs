//! MPI SSSP benchmark binary. Each rank loads the full weighted graph and
//! participates in collective Allreduce steps. Run with:
//!
//!   mpirun -np <p> -H compute6,compute7 ./sssp-mpi <graph_file> <num_nodes> <source> <max_iter>

use mpi::traits::*;
use sssp_mpi::run_bellman_ford_mpi;
...
fn main() {
    ...
    if args.len() < 5 {
        if rank == 0 {
            eprintln!("Usage: {} <graph_file> <num_nodes> <source> <max_iter>", args[0]);
        }
        std::process::exit(1);
    }
    ...
    let result = run_bellman_ford_mpi(&world, &csr, source, max_iter);
