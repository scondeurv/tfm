...
STANDALONE_BINARY = str(HERE / "sssp-standalone" / "target" / "release" / "sssp-standalone")
RAYON_BINARY = str(HERE / "sssp-rayon" / "target" / "release" / "sssp-rayon")
MPI_BINARY = str(HERE / "sssp-mpi" / "target" / "release" / "sssp-mpi")
...
def benchmark_standalone(graph_file: str, num_nodes: int, source_node: int, max_iterations: int, timeout: int = 600):
    """Run the standalone (single-thread CSR) SSSP binary."""
    ...
    if not os.path.exists(graph_file):
        ...
        return None
    cmd = [STANDALONE_BINARY, graph_file, str(num_nodes), str(source_node), str(max_iterations)]
    return _run_single_node_binary(cmd, "standalone", timeout)


def benchmark_rayon(graph_file: str, num_nodes: int, source_node: int, max_iterations: int, threads=None, timeout: int = 600):
    """Run Rayon (multi-thread shared-memory CSR) SSSP."""
    if not os.path.exists(RAYON_BINARY):
        print(
            f"Error: Binary not found at {RAYON_BINARY}\n"
            "Run: cd sssp-rayon && cargo build --release",
            file=sys.stderr,
        )
        return None
    if not os.path.exists(graph_file):
        print(f"Error: Graph file not found: {graph_file}", file=sys.stderr)
        return None
    cmd = [RAYON_BINARY, graph_file, str(num_nodes), str(source_node), str(max_iterations)]
    if threads is not None:
        cmd.append(str(threads))
    return _run_single_node_binary(cmd, "rayon", timeout)


def benchmark_mpi(graph_file: str, num_nodes: int, source_node: int, max_iterations: int, ranks: int, hosts=None, timeout: int = 600):
    """Run MPI (distributed CSR via Allreduce-MIN) SSSP."""
    if not os.path.exists(MPI_BINARY):
        print(
            f"Error: Binary not found at {MPI_BINARY}\n"
            "Run: cd sssp-mpi && cargo build --release",
            file=sys.stderr,
        )
        return None
    if not os.path.exists(graph_file):
    ...
    if hosts:
        cmd += ["-H", hosts]
    cmd += [MPI_BINARY, graph_file, str(num_nodes), str(source_node), str(max_iterations)]
...
if __name__ == "__main__":
    ...
    if args.run_rayon:
        ...
        rayon_output = benchmark_rayon(
            graph_file, args.nodes, args.source, args.max_iterations, args.rayon_threads,
        )
    ...
    if args.run_mpi:
        ...
        mpi_output = benchmark_mpi(
            graph_file, args.nodes, args.source, args.max_iterations, ranks, args.mpi_hosts,
        )
