...
ACTION_DIR="$SCRIPT_DIR/ow-bfs"

# Prefer the middleware copy inside the bfs repo. Fall back to sibling if needed.
...
echo "🚀 Starting compilation of BFS action using cluster-identical environment..."
...
zip -j "$SCRIPT_DIR/bfs.zip" "$ACTION_DIR/bin/exec"
echo "📦 bfs.zip is ready: $SCRIPT_DIR/bfs.zip"
