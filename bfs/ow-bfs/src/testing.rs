use actions::{main as ow_main, FrontierMessage};
...
/// BFS distributed testing executable (local Redis, no OpenWhisk)
#[derive(Parser, Debug)]
...
fn main() {
    ...
    let actors = proxies
        ...
        .collect::<HashMap<u32, Middleware<FrontierMessage>>>();

    let mut actors_vec = actors.into_iter().collect::<Vec<_>>();
    // Sort by worker_id so order matches the ordered params list
