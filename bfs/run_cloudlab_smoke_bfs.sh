...
PYTHON_BIN="${BFS_PYTHON_BIN:-${SCRIPT_DIR}/.venv/bin/python}"
NODES="${BFS_SMOKE_NODES:-100000}"
PARTITIONS="${BFS_SMOKE_PARTITIONS:-4}"
GRANULARITY="${BFS_SMOKE_GRANULARITY:-2}"
SOURCE_NODE="${BFS_SMOKE_SOURCE:-0}"
MAX_LEVELS="${BFS_SMOKE_MAX_LEVELS:-500}"
MEMORY_MB="${BFS_SMOKE_MEMORY_MB:-4096}"
OW_HOST="${OW_HOST:-127.0.0.1}"
OW_PORT="${OW_PORT:-31001}"
OW_NAMESPACE="${OW_NAMESPACE:-openwhisk}"
OW_RELEASE_NAME="${OW_RELEASE_NAME:-owdev}"
BUCKET="${BFS_SMOKE_BUCKET:-tfm-smoke}"
KEY_PREFIX="${BFS_SMOKE_KEY_PREFIX:-cloudlab/bfs}"
...
if [[ ! -f "${SCRIPT_DIR}/bfs.zip" ]]; then
  echo "Missing ${SCRIPT_DIR}/bfs.zip; compile the Burst action first" >&2
  exit 1
fi
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/benchmark_bfs.py" \
  --nodes "${NODES}" \
