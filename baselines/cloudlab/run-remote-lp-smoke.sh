...
APP_NAME="${SPARK_APP_NAME:-tfm-lp-cell}"
...
kubectl -n "${NAMESPACE}" exec "${MASTER_POD}" -- bash -lc "
  ...
  /opt/spark/bin/spark-shell \
    ...
    --driver-java-options '-Dtfm.input=s3a://${BUCKET}/${INPUT_KEY} -Dtfm.output=s3a://${BUCKET}/${OUTPUT_PREFIX} -Dtfm.max_iter=${MAX_ITER} -Dtfm.partitions=${PARTITIONS} -Dtfm.persist=true' \
    -i /opt/tfm-spark/scripts/labelpropagation_graphx_shell.scala
"
