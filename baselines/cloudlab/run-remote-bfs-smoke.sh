...
MAX_LEVELS="${7:?max_levels required}"
...
APP_NAME="${SPARK_APP_NAME:-tfm-bfs-cell}"
...
kubectl -n "${NAMESPACE}" exec "${MASTER_POD}" -- bash -lc "
  ...
  /opt/spark/bin/spark-shell \
    ...
    --driver-java-options '-Dtfm.input=s3a://${BUCKET}/${INPUT_KEY} -Dtfm.output=s3a://${BUCKET}/${OUTPUT_PREFIX} -Dtfm.source=${SOURCE_NODE} -Dtfm.max_levels=${MAX_LEVELS} -Dtfm.partitions=${PARTITIONS} -Dtfm.persist=true' \
    -i /opt/tfm-spark/scripts/bfs_graphx_shell.scala
"
