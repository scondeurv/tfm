...
DAMPING="${PAGERANK_DAMPING:-0.85}"
TOLERANCE="${PAGERANK_TOLERANCE:-1e-8}"
...
docker exec spark-master /opt/spark/bin/spark-shell \
  ...
  --driver-java-options "-Dtfm.input=$INPUT_PATH -Dtfm.output=$OUTPUT_PATH -Dtfm.max_iter=$MAX_ITER -Dtfm.partitions=$PARTITIONS -Dtfm.damping=$DAMPING -Dtfm.tolerance=$TOLERANCE -Dtfm.persist=$PERSIST_OUTPUT" \
  -i "$SCRIPT_PATH"
