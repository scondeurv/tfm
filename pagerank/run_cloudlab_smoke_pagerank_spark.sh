#!/usr/bin/env bash
# PageRank Spark/GraphX smoke driver.
#
# Mirrors labelpropagation/run_cloudlab_smoke_lp_spark.sh: deploys the
# Spark cluster on CloudLab via baselines/cloudlab/deploy-spark-smoke.sh,
# uploads the graph file to MinIO, executes pagerank_graphx_shell.scala
# inside the master pod, and prints the RESULT_JSON emitted by the Scala
# job.
#
# Validation against the standalone PR run is intentionally skipped (the
# orchestrator sets SPARK_SKIP_VALIDATION=true) because we don't ship a
# PR-specific Python validator.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
PYTHON_BIN="${PR_PYTHON_BIN:-${SCRIPT_DIR}/.venv/bin/python}"
...
INPUT_KEY="${SPARK_SMOKE_INPUT_KEY:-cloudlab/spark/pagerank/large_100000.txt}"
OUTPUT_PREFIX="${SPARK_SMOKE_OUTPUT_PREFIX:-cloudlab/spark/pagerank/large-100000/output}"
MAX_ITER="${PAGERANK_SPARK_SMOKE_MAX_ITER:-100}"
PARTITIONS="${PAGERANK_SPARK_SMOKE_PARTITIONS:-4}"
GRAPH_FILE="${PAGERANK_SPARK_SMOKE_GRAPH_FILE:-${SCRIPT_DIR}/large_pagerank_100000.txt}"
NODES="${PAGERANK_SPARK_SMOKE_NODES:-100000}"
DAMPING="${PAGERANK_SPARK_SMOKE_DAMPING:-0.85}"
TOLERANCE="${PAGERANK_SPARK_SMOKE_TOLERANCE:-1e-6}"
...
REMOTE_RUNNER="${REMOTE_RUNNER:-/tmp/${SPARK_NAMESPACE}/run-remote-pagerank-smoke.sh}"
REMOTE_BASE="${REMOTE_BASE:-/tmp/${SPARK_NAMESPACE}}"
REMOTE_GRAPH_FILE="${REMOTE_BASE}/$(basename "${GRAPH_FILE}")"
LOCAL_OUTPUT_BASE="$(mktemp -d /tmp/${SPARK_NAMESPACE}-pr-local-XXXXXX)"
...
scp -i "${CLOUDLAB_SSH_KEY}" \
  "${ROOT_DIR}/baselines/cloudlab/run-remote-pagerank-smoke.sh" \
