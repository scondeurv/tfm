//! Rayon LP benchmark binary. Same CLI surface as `lpst` so the campaign
//! driver can swap backends with a single flag.

use label_propagation::build_csr_from_edges;
use lp_rayon::run_lp_rayon;
...
fn main() {
    ...
    if args.len() < 4 {
        eprintln!("Usage: {} <graph_file> <num_nodes> <max_iter> [threads]", args[0]);
        std::process::exit(1);
    }
    ...
    let labels = run_lp_rayon(&csr, &initial_labels, max_iter);
